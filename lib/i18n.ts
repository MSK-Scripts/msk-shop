...
export const statsTranslations = {
  en: {
    ...
    card_giveaway_keys_sub:  'Paid keys handed out as giveaway prizes',
    card_sponsored_keys:     'Sponsored keys',
    card_sponsored_keys_sub: 'Paid keys granted free of charge',
  ...
  de: {
    ...
    card_giveaway_keys:      'Giveaway API Keys',
    card_giveaway_keys_sub:  'Bezahlte Keys als Giveaway-Gewinn vergeben',
    card_sponsored_keys:     'Sponsoring-Keys',
    card_sponsored_keys_sub: 'Bezahlte Keys kostenlos vergeben',
...
export const dashboardTranslations = {
  en: {
    ...
    host_intro:           'We install the ticket bot on our server, keep it running and update it. You supply three values from the Discord developer portal, we take care of everything else.',
    host_step1:           'Open the Discord developer portal and pick your application, or create one.',
    host_step2:           'Under "Bot" copy the token. If you have never revealed it, use "Reset Token". Discord shows a token only once.',
    host_step3:           'Under "OAuth2" copy the Client ID and the Client Secret. The dashboard login needs them.',
    host_step4:           'Invite the bot to your server with the Administrator permission.',
    host_after:           'After activation you get one more address to register in Discord. We show it to you then.',
    host_token:           'Bot token',
    host_token_hint:      'Developer portal → your application → Bot → Token',
    host_client_id:       'Client ID',
    host_client_id_hint:  'Developer portal → your application → OAuth2 → Client ID',
    host_client_secret:   'Client secret',
    host_client_secret_hint: 'Developer portal → your application → OAuth2 → Client Secret',
    host_database:        'Own database (optional)',
    host_database_hint:   'Leave empty to use the bundled file database. Otherwise a mysql://, postgres:// or sqlite: URL.',
    host_portal:          'Let members see their own tickets in the dashboard',
    host_portal_hint:     'Off by default: the dashboard is for your team. Turned on, any member can sign in and see only their own tickets.',
    host_secret_set:      'Set. Leave empty to keep it.',
    host_activate:        'Activate bot hosting',
    host_activating:      'Setting up…',
    host_save_restart:    'Save and restart',
    host_required:        'Please fill in all three required fields.',
    host_step_queued:     'Preparing…',
    host_step_clone:      'Downloading the bot…',
    host_step_configure:  'Writing the configuration…',
    host_step_install:    'Installing dependencies, this takes a few minutes…',
    host_step_start:      'Starting the bot…',
    host_step_health:     'Waiting for the bot to come up…',
    host_step_done:       'Done.',
    host_running_note:    'You can close this page. The setup keeps running and you will see the result when you come back.',
    host_failed_title:    'The bot did not start',
    host_failed_hint:     'The log above names the cause. If it points at one of the values below, correct it and save; the bot is reinstalled only if that is really necessary.',
    host_log_title:       'Last lines from the bot',
    host_err_invalid_token:         'That does not look like a bot token. Copy it under Bot → Token.',
    host_err_invalid_client_id:     'A client ID is 17 to 20 digits.',
    host_err_invalid_client_secret: 'That does not look like a client secret. Copy it under OAuth2 → Client Secret.',
    host_err_invalid_database_url:  'A database URL starts with mysql://, postgres:// or sqlite:.',
    host_needs_config_title: 'Almost there: the bot still needs its channels and roles',
    host_needs_config_desc:  'Hosting is running and the bot is online, but it is still holding the example values for channels, categories and roles. Until you replace them it will not open any tickets.',
    host_needs_config_link:  'Open the bot dashboard and finish the setup',
    host_err_bot_not_in_guild:      'This bot is not on your server yet. Invite it with the Administrator permission, then try again.',
    host_err_health_not_running:    'The bot was installed but stopped again right away. Discord usually rejects the token, so check it below.',
    host_err_health_unreachable:    'The bot was installed but did not answer. Please try again, and contact us if it keeps happening.',
    host_err_in_progress:           'A setup is already running for this server.',
    ...
    // ── Own domain in front of the bot dashboard ─────────────────────────────
    dashdom_title:        'Own domain for the bot dashboard',
    dashdom_desc:         'Serve your bot dashboard under your own address instead of the one we generated. The generated address keeps working.',
    dashdom_placeholder:  'tickets.yourserver.com',
    dashdom_btn:          'Use this domain',
    dashdom_btn_loading:  'Setting up…',
    dashdom_needs_host:   'Available once bot hosting is set up.',
    dashdom_redirect_warn: 'The dashboard login only works once you add this URL in the Discord developer portal under OAuth2 → Redirects. The old one keeps working, so you can leave it in place.',
    dashdom_removed:      'Your dashboard is back on the address we generated.',
    dashdom_err_invalid_domain:  'Enter a domain like tickets.yourserver.com, without https:// and without a path.',
    dashdom_err_reserved_domain: 'That domain belongs to us. Use one you own.',
    dashdom_err_domain_taken:    'This domain is already in use for another server or for transcripts.',
    dashdom_err_ssl_failed:      'Could not issue a certificate. The A-record has to point at our server and stay that way while the certificate is issued.',
    dashdom_err_setup_failed:    'Could not set the domain up. Please try again.',
    dashdom_err_not_hosted:      'Set up bot hosting first.',
    ...
    host_archive_restore_hint: 'Your tickets and settings are kept. The bot is updated and started with the values you just entered.',
    host_archive_new:     'Start fresh',
    host_archive_new_hint: 'The archived installation is deleted for good, including the ticket history. This cannot be undone.',
    host_archive_cancel:  'Not now',
    host_remove_title:    'Remove bot hosting',
    host_remove_desc:     'Stops the bot, takes its address offline and archives the installation. Your ticket history is kept for 14 days and then deleted for good.',
    host_remove:          'Remove hosting',
    host_remove_confirm:  'Type the server ID to confirm:',
    host_remove_busy:     'Removing…',
    host_removed:         'Bot hosting has been removed.',
    botdash_addr_title:   'Dashboard address',
    botdash_addr_desc:    'This is your bot dashboard. Share it with your team: they sign in with Discord and see exactly what their permissions allow.',
    botdash_addr_none:    'This bot does not have a public address yet. Set one up to let your team sign in with their own Discord account.',
    botdash_setup:        'Set up address',
    botdash_setup_busy:   'Setting up…',
    botdash_setup_done:   'The address is live. It may take a few minutes before it resolves everywhere.',
    botdash_redirect_title: 'One step in Discord, and only you can do it',
    botdash_redirect_desc:  'Open the Discord developer portal, pick your application, go to OAuth2 → Redirects and add this URL. Without it the login ends on a Discord error page.',
    botdash_redirect_open:  'Open developer portal',
    botdash_restart_note:  'The bot restarts once so it picks up its new address.',
    botdash_copy:         'Copy',
    botdash_copied:       'Copied',
    botdash_copy_failed:  'Could not copy automatically. Please select the text and copy it by hand.',
    botdash_fallback:     'Owner access without a Discord login',
    botdash_fallback_hint:'Opens the dashboard through msk-scripts.de. Use this if the Discord login is not working yet. It only ever works for you, not for your team.',
    dash_copied:          'Copied',
  ...
  de: {
    ...
    host_step2:           'Unter "Bot" den Token kopieren. Wenn du ihn noch nie angezeigt hast, nimm "Reset Token". Discord zeigt einen Token nur ein einziges Mal.',
    host_step3:           'Unter "OAuth2" die Client ID und das Client Secret kopieren. Die braucht der Login ins Dashboard.',
    host_step4:           'Lade den Bot mit Administrator-Recht auf deinen Server ein.',
    host_after:           'Nach der Aktivierung bekommst du noch eine Adresse, die du in Discord eintragen musst. Wir zeigen sie dir dann.',
    host_token:           'Bot-Token',
    host_token_hint:      'Developer Portal → deine Anwendung → Bot → Token',
    host_client_id:       'Client ID',
    host_client_id_hint:  'Developer Portal → deine Anwendung → OAuth2 → Client ID',
    host_client_secret:   'Client Secret',
    host_client_secret_hint: 'Developer Portal → deine Anwendung → OAuth2 → Client Secret',
    host_database:        'Eigene Datenbank (optional)',
    host_database_hint:   'Leer lassen für die mitgelieferte Dateidatenbank. Sonst eine mysql://-, postgres://- oder sqlite:-Adresse.',
    host_portal:          'Mitglieder ihre eigenen Tickets im Dashboard sehen lassen',
    host_portal_hint:     'Standardmäßig aus, das Dashboard ist für dein Team. Eingeschaltet kann sich jedes Mitglied anmelden und sieht ausschließlich seine eigenen Tickets.',
    host_secret_set:      'Gesetzt. Leer lassen, um ihn zu behalten.',
    host_activate:        'Bot Hosting aktivieren',
    host_activating:      'Wird eingerichtet…',
    host_save_restart:    'Speichern und neu starten',
    host_required:        'Bitte alle drei Pflichtfelder ausfüllen.',
    host_step_queued:     'Vorbereitung…',
    host_step_clone:      'Bot wird heruntergeladen…',
    host_step_configure:  'Konfiguration wird geschrieben…',
    host_step_install:    'Abhängigkeiten werden installiert, das dauert ein paar Minuten…',
    host_step_start:      'Bot wird gestartet…',
    host_step_health:     'Warten, bis der Bot oben ist…',
    host_step_done:       'Fertig.',
    host_running_note:    'Du kannst die Seite schließen. Die Einrichtung läuft weiter, das Ergebnis siehst du beim nächsten Besuch.',
    host_failed_title:    'Der Bot ist nicht gestartet',
    host_failed_hint:     'Im Log oben steht die Ursache. Zeigt sie auf einen der Werte unten, korrigier ihn und speichere; neu installiert wird nur, wenn es wirklich nötig ist.',
    host_log_title:       'Letzte Zeilen des Bots',
    host_err_invalid_token:         'Das sieht nicht nach einem Bot-Token aus. Kopiere ihn unter Bot → Token.',
    host_err_invalid_client_id:     'Eine Client ID besteht aus 17 bis 20 Ziffern.',
    host_err_invalid_client_secret: 'Das sieht nicht nach einem Client Secret aus. Kopiere es unter OAuth2 → Client Secret.',
    host_err_invalid_database_url:  'Eine Datenbankadresse beginnt mit mysql://, postgres:// oder sqlite:.',
    host_needs_config_title: 'Fast fertig: dem Bot fehlen noch Kanäle und Rollen',
    host_needs_config_desc:  'Das Hosting läuft und der Bot ist online, er trägt aber noch die Beispielwerte für Kanäle, Kategorien und Rollen. Solange die drin stehen, öffnet er keine Tickets.',
    host_needs_config_link:  'Bot-Dashboard öffnen und Einrichtung abschließen',
    host_err_bot_not_in_guild:      'Der Bot ist noch nicht auf deinem Server. Lade ihn mit der Administrator-Berechtigung ein und versuch es dann noch einmal.',
    host_err_health_not_running:    'Der Bot wurde installiert, hat sich aber sofort wieder beendet. Meistens lehnt Discord den Token ab, prüf ihn unten.',
    host_err_health_unreachable:    'Der Bot wurde installiert, hat aber nicht geantwortet. Versuch es bitte noch einmal und melde dich, wenn es wieder passiert.',
    ...
    dashdom_title:        'Eigene Domain für das Bot-Dashboard',
    dashdom_desc:         'Liefere dein Bot-Dashboard unter deiner eigenen Adresse aus statt unter der, die wir vergeben haben. Die vergebene Adresse funktioniert weiterhin.',
    dashdom_placeholder:  'tickets.deinserver.de',
    dashdom_btn:          'Diese Domain verwenden',
    dashdom_btn_loading:  'Wird eingerichtet…',
    dashdom_needs_host:   'Verfügbar, sobald das Bot Hosting eingerichtet ist.',
    dashdom_redirect_warn: 'Der Login ins Dashboard funktioniert erst, wenn du diese URL im Discord Developer Portal unter OAuth2 → Redirects einträgst. Die alte bleibt gültig, du kannst sie also stehen lassen.',
    dashdom_removed:      'Dein Dashboard läuft wieder unter der Adresse, die wir vergeben haben.',
    dashdom_err_invalid_domain:  'Trage eine Domain wie tickets.deinserver.de ein, ohne https:// und ohne Pfad.',
    dashdom_err_reserved_domain: 'Diese Domain gehört uns. Nimm eine, die dir gehört.',
    dashdom_err_domain_taken:    'Diese Domain wird bereits für einen anderen Server oder für Transkripte verwendet.',
    dashdom_err_ssl_failed:      'Das Zertifikat konnte nicht ausgestellt werden. Der A-Record muss auf unseren Server zeigen und das auch während der Ausstellung bleiben.',
    dashdom_err_setup_failed:    'Die Domain konnte nicht eingerichtet werden. Bitte erneut versuchen.',
    dashdom_err_not_hosted:      'Richte zuerst das Bot Hosting ein.',
    ...
    host_archive_restore_hint: 'Deine Tickets und Einstellungen bleiben erhalten. Der Bot wird aktualisiert und mit den Werten gestartet, die du gerade eingegeben hast.',
    host_archive_new:     'Neu anfangen',
    host_archive_new_hint: 'Die archivierte Installation wird endgültig gelöscht, samt Ticket-Historie. Das lässt sich nicht rückgängig machen.',
    host_archive_cancel:  'Jetzt nicht',
    host_remove_title:    'Bot Hosting entfernen',
    host_remove_desc:     'Stoppt den Bot, nimmt seine Adresse vom Netz und archiviert die Installation. Deine Ticket-Historie bleibt 14 Tage erhalten und wird danach endgültig gelöscht.',
    host_remove:          'Hosting entfernen',
    host_remove_confirm:  'Zum Bestätigen die Server-ID eintippen:',
    host_remove_busy:     'Wird entfernt…',
    host_removed:         'Das Bot Hosting wurde entfernt.',
    botdash_addr_title:   'Adresse des Dashboards',
    botdash_addr_desc:    'Das ist dein Bot-Dashboard. Gib die Adresse an dein Team weiter: es meldet sich mit Discord an und sieht genau das, was seine Rechte erlauben.',
    botdash_addr_none:    'Dieser Bot hat noch keine öffentliche Adresse. Richte eine ein, damit sich dein Team mit dem eigenen Discord-Konto anmelden kann.',
    botdash_setup:        'Adresse einrichten',
    botdash_setup_busy:   'Wird eingerichtet…',
    botdash_setup_done:   'Die Adresse ist aktiv. Bis sie überall erreichbar ist, können ein paar Minuten vergehen.',
    botdash_redirect_title: 'Ein Schritt in Discord, den nur du machen kannst',
    botdash_redirect_desc:  'Öffne das Discord Developer Portal, wähle deine Anwendung, geh auf OAuth2 → Redirects und trage diese URL ein. Ohne sie endet die Anmeldung auf einer Discord-Fehlerseite.',
    botdash_redirect_open:  'Developer Portal öffnen',
    botdash_restart_note:  'Der Bot startet einmal neu, damit er seine neue Adresse übernimmt.',
    botdash_copy:         'Kopieren',
    botdash_copied:       'Kopiert',
    botdash_copy_failed:  'Automatisches Kopieren ging nicht. Bitte den Text markieren und von Hand kopieren.',
    botdash_fallback:     'Besitzer-Zugang ohne Discord-Login',
    botdash_fallback_hint:'Öffnet das Dashboard über msk-scripts.de. Nimm das, solange der Discord-Login noch nicht funktioniert. Es geht nur für dich, nicht für dein Team.',
    dash_copied:          'Kopiert',
...
export const giveawayDashboardTranslations = {
  en: {
    ...
    f_draw_mode:     'Winner selection',
    draw_random:     'Random draw when it ends',
    draw_first_click: 'First click wins',
    draw_first_click_hint: 'No draw: the first entries take the prize and the giveaway ends right there. The duration becomes a deadline for the case that nobody clicks. Bonus entries have no effect in this mode, and an entry cannot be withdrawn.',
    draw_first_click_edit_hint: 'Switching does not end anything. Entries that already exist keep their order, so the earliest one wins, and the giveaway ends on the next click or at the deadline.',
    draw_badge_fast: 'First click',
    prizes_label:    'Prizes:',
    f_winners:     'Winners',
    f_duration:    'Duration',
    btn_create:    'Create',
  ...
  de: {
    ...
    f_draw_mode:     'Gewinner-Ermittlung',
    draw_random:     'Ziehung am Ende',
    draw_first_click: 'Wer zuerst klickt, gewinnt',
    draw_first_click_hint: 'Keine Ziehung: Die ersten Teilnahmen bekommen den Preis, und das Giveaway ist damit sofort vorbei. Die Dauer wird zur Frist für den Fall, dass niemand klickt. Bonus-Lose wirken in diesem Modus nicht, und die Teilnahme lässt sich nicht zurücknehmen.',
    draw_first_click_edit_hint: 'Das Umstellen beendet nichts. Vorhandene Teilnahmen behalten ihre Reihenfolge, es gewinnt also die früheste, und beendet wird beim nächsten Klick oder zur Frist.',
    draw_badge_fast: 'Erster Klick',
    prizes_label:    'Preise:',
    f_winners:     'Gewinner',
    f_duration:    'Dauer',
